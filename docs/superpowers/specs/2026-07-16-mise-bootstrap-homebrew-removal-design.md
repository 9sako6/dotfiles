...
1. prune validation と top-level stale path の退避について、失敗する test を先に追加する。
2. 隔離 HOME と fake mise を使う bootstrap E2E test を追加し、clone 後の順序が `trust`、repo bootstrap だけになることを観測する。
3. ユーザー config の bootstrap を隔離 HOME で実行し、CLI backend の解決と executable を確認する。外部 package の実インストールは dry-run/status で検証する。
4. `mise run dev:test` を実行する。
