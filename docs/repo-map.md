...
- 実装責務の分担：新規dotfiles固有の状態判断、排他、反映制御、記録更新および対応テストはRust CLIを実装の所有者とする。shellの責務は初回導入と必要な薄い外部プロセス接続にとどめ、Nix構成宣言はNixが担う。既存コード全体の一括移行は前提とせず、追加・変更する責務から順次Rustへ寄せる。標準APIと既存のRust依存クレートを優先し、OS連携に必要な追加のRust依存を採用する場合は理由を明確にする。
...
- 公開flake.nixを引き続き唯一の構成ルートとし、dotfiles CLIは公開・非公開Gitの固定スナップショットハッシュ、非機密ローカルTOML、ユーザー名、作業ディレクトリ、公開revisionをまとめた評価用flakeをNix storeに生成して公開lib.mkHostを呼び出す構成とする。
- 入力内容に変更がない場合はNix標準の評価キャッシュを再利用し、入力変更時やキャッシュ削除時のみ再評価を行う設計とし、lockファイルの更新は行わない。
- planは固定した公開・非公開ソースと設定入力を一時GCルートで保持し、宣言inventoryから差分とcopy配備計画を作成する。通常は世代のビルドを行わず、宣言一覧が同一であれば予定出力パスで比較し、旧形式の世代に対してのみ既存のネイティブ差分へフォールバックする。稼働中のシステムやホーム配置は変更しない。
- applyは排他ロックを取得し、差分へのyes確認後に固定入力から世代をビルドする。ビルドの前後に構成入力とアクティブ世代を検証して変化があれば反映せず中断し、変化がなければその世代を反映する。確認以前にビルド済みのフォールバック生成物は再利用する。
