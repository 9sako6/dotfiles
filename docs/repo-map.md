...
- URL 指定や `--default` フラグによる動的ソース選択は廃止され、古い構文に対しては移行案内を出力する。ローカル設定が存在しない状態で旧世代の非公開設定が有効である場合は、外部サービスを自動削除せず処理を停止する。
...
- 自動テストでは未起動時のエラー処理やネットワーク遮断を検証する。実機 GPU による推論テストは大容量ダウンロードを伴うため CI では実行せず、CI 上でのシステム反映やモデル取得は禁止する。
