...
- `install.sh` は新しい Mac を一発で構築する唯一の入口とし、repo tools、system、home、user tools と repositories の順序を所有する。
- `.mise.toml` の task は日常的に個別実行する操作だけを公開し、bootstrap の順序を再構成しない。
- `system:plan` は Lix や active system を変更しない。`system:apply` だけが Lix 導入、build 済み世代の activation、source 選択の永続化を行う。
