...
planとapplyは反映済みのNix世代と今回ビルドした世代の一覧にある宣言情報を比較し、settingsと共通の表形式で変更行のみ赤い-と緑の+で表示します。latestの取得や比較は行わず、一覧やスキルの元データを世代のNix store参照として保持するため、ロールバックすれば比較元も戻ります。あわせてNixの依存閉包差分、実際のHomebrew操作候補、copy計画も引き続き表示します。

一覧を含まない古い世代からの初回実行時は比較不能を明示し、全件追加や差分なしとしては扱わずに既存のNixやHomebrewの差分を用います。新形式の世代をapplyした次回から表の差分が有効になります。また、applyの確認後には反映済み世代が変わっていないことを再検証し、変更されていた場合は処理を停止します。

Night Shiftと音声入力ショートカットの設定はnix/macos-settings.nixを宣言元とし、反映スクリプトと一覧JSONの双方が同じ宣言を参照します。今後スクリプト経由の設定を追加する場合もスクリプト単体への値の埋め込みを避け、値の宣言とともに一覧化と差分検証を組み合わせてください。旧形式の一覧には過去値が存在せず形式の異なる世代間は比較できないため、初回の反映までは既存のNixやHomebrewの差分を確認し、新形式の反映完了後から設定一覧の差分表で検証します。
