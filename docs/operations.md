...
Nixとmiseは固定版と最新安定版を比較し、Homebrew（nix-darwin）は固定なしで表示します。端末環境で実行すると、設定、パッケージ、システム、サービス、エージェントの全一覧が1つのスクロール画面に表示されます。current列は宣言された構成を示し、latest列はバックグラウンドで最大12秒照会され、取得できた行から順次更新されます。取得失敗時はエラー理由が表示され、照会完了後も画面は自動で閉じません。qまたはEscで正常終了して通常画面に戻り、終了後に一覧の再出力は行いません（Ctrl+Cは終了コード130で中断）。標準出力をパイプやファイルへ渡す場合、入力が端末でない場合、TERMが未設定またはdumbの場合は、latest列と照会を省いて通常テキストを一度だけ出力します。

`j`/`k`や上下キーで1行、`f`/`Space`で次ページ、`b`で前ページへ移動します。`d`/`u`での半ページ移動や`g`/`G`での先頭・末尾移動に加え、`Ctrl+F`/`Ctrl+B`/`Ctrl+D`/`Ctrl+U`にも対応しています。

`plan`および`apply`は、実機の手動変更の検知ではなく、反映済みNix世代と計画世代の一覧に記述された宣言情報同士を比較します。差分がある場合は`settings`と同じ表形式を用いて、変更行のみを赤の`-`と緑の`+`で表示します。リビジョン、ハッシュ、Nix世代パス、変更のないHomebrewの見出し、全件のcopy予定などの内部情報は通常プレビューから省かれます。Night Shiftや音声入力ショートカットの設定は、`nix/macos-settings.nix`の共通宣言を反映し、一覧からも参照されます。

画面は`settings`と同様のスクロール画面で開き、`q`または`Esc`で閉じ、`Ctrl+C`を押すとステータスコード130でコマンド全体を中止します。パイプ処理時や`TERM`環境変数の未設定、`dumb`端末では専用画面を開かず差分テキストのみを出力し、差分がない場合は画面も開かず何も出力しません。また、古い世代に一覧が存在しない場合や形式が異なる場合に限り比較不能の旨を伝え、従来のNixおよびHomebrewの差分を同画面に表示します。共通表による比較は、新形式を反映した世代以降で有効になります。

`apply`では、差分画面を閉じた後に従来の`yes`確認プロンプトを表示します。確認後に反映済み世代の変更が検知された場合は、競合を防ぐため処理を中断します。外部への最新情報の照会は行われません。
