...
describe("シェル設定", () => {
  ...
  test("zshenvはNixのsystem packageをmacOS標準コマンドより先に解決する", async () => {
    const result = await runCommand("zsh", [
      "-f",
      "-c",
      "source dist/.zshenv; printf '%s %s' $path[(i)/run/current-system/sw/bin] $path[(i)/usr/bin]",
    ]);
    ...
    await withTempDir("zshrc-nix-path", async (tempDir) => {
      ...
      const result = await runCommand(
        ...
        [
          ...
          "-i",
          "-c",
          "source dist/.zshrc; printf '%s %s' $path[(i)/run/current-system/sw/bin] $path[(i)/usr/bin]",
        ],
        {
          ...process.env,
