...
if [ -z "$recent" ]; then
  printf '%s\n' "logging-agents: 直近${THRESHOLD_SECONDS}秒以内に event file がありません。logging-agents skill を発動し、write-event.sh で summary、evidence、next action を自由記述してから続行してください。" >&2
  exit 2
fi
exit 0
