...
fn inspect_selected_system_source(
    ...
    let (request, target) = inspect_selection(
        &accepted_public_flakes,
        data_root,
        selection_path,
    )?;
...
fn inspect_selection(
    ...
    if public_flakes.iter().any(|flake| *flake == target) {
...
fn prepare_remote_checkout(data_root: &Path, git_url: &str) -> Result<(PathBuf, String)> {
    ...
    let result = (|| -> Result<String> {
        ...
        git_run(Some(&directory), &["remote", "set-head", "origin", "--auto"])?;
        let revision = git_capture(
            Some(&directory),
...
fn is_hex_digest_prefix(value: &str) -> bool {
    value.len() == 24 && value.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase())
}
...
mod tests {
    ...
    use super::{managed_checkout_path, sha256_hex, source_request, system_source_data_root, validate_git_url, SourceRequest};
    ...
    fn matches_previous_managed_checkout_digest() {
        let path = managed_checkout_path(
            Path::new("/tmp/data"),
            "git@example.test:owner/config.git",
        );
